...
if (isset($_SESSION["isAuthenticated"])) {
    $queryAccounts = $connection->prepare("SELECT * FROM accounts WHERE deleted_at IS NULL AND user_id = :user_id");
    $queryAccounts->execute(["user_id" => $_SESSION["user_id"]]);
    ...
    if (isset($_POST["submit_account"])) {
        ...
        $insertAccount = $connection->prepare("INSERT INTO accounts (user_id, account_name, account_type, starting_amount, currency_type) VALUES (:user_id, :account_name, :account_type, :starting_amount, :currency_type)");

        $insertAccount->execute(["user_id" => $_SESSION["user_id"], "account_name" => $account_name, "account_type" => $account_type, "starting_amount" => $starting_amount, "currency_type" => $currency_type]);
...
<body>
            ...
            <div id="container">
                <ul class="breadcrumb">
                    <li style="display: flex; flex-direction: row; padding: 5px 0">
                        <i class="ti ti-wallet breadcrumb-list"></i>
                        <div style="height: fit-content; margin: auto 0">
                            <a class="margin-y-auto" href="#"><span style="font-size: 16px">Account</span></a>
                        </div>
                    </li>
                </ul>
                ...
                <div style="background-color: #ffffff; margin-top: 15px; border-radius: 5px; padding: 10px 15px">
                    <cds-table is-sortable>
                        <cds-table-header-title slot="title" style="color: #0068a3; font-weight: 600; margin-left: -14px">Account List</cds-table-header-title>
                        ...
                        <cds-table-toolbar slot="toolbar">
                            <div style="display: flex; justify-content: space-between">
                                ...
                                <div style="display: flex; flex-direction: row; gap: 10px">
                                    ...
                                    <button id="modal-add-account" style="
                                                display: flex;
                                                flex-direction: row;
                                                background-color: transparent;
                                                background-color: #0068a3;
                        ...
                        <cds-table-head>
                            <cds-table-header-row>
                                ...
                                <cds-table-header-cell>Account Type</cds-table-header-cell>
                                <cds-table-header-cell>Account Initial Amount</cds-table-header-cell>
                                <cds-table-header-cell>Actions</cds-table-header-cell>
                            </cds-table-header-row>
                        </cds-table-head>
                        <cds-table-body>
                            <?php foreach ($queryAccounts as $key => $value) { ?>
                                <cds-table-row>
                                    <cds-table-cell style="color: #00AA70"><?php echo $value["account_name"] ?></cds-table-cell>
                                    <cds-table-cell style="color: #00AA70"><?php echo $value["account_type"] ?></cds-table-cell>
                                    <cds-table-cell style="color: #00AA70">+ <?php echo $value["currency_type"] . " " . $value["starting_amount"] ?></cds-table-cell>
                                    <cds-table-cell style="display: flex; flex-direction: row; gap: 5px">
                                        <button style="
                                                ...
                                                margin: 10px 0;
                                            ">
                                            <i class="ti ti-eye plus-icon-color"></i>
                                            <p style="margin: auto 0; height: fit-content; color: #ffffff">
                                                View
                                            </p>
                                        </button>
                                                ...
                                                <div style="background-color: #ffffff; width: 50%; margin: 0 auto;">
                                                    ...
                                                    <cds-modal-body>
                                                        ...
                                                        <p style="font-weight: 400; margin: 0 30px; font-size: 16px">Do you want to delete the account name
                                                            <span style="font-weight: 600; color: #FE3B2C" id="account_name"></span>
                                                            ?
                                                        </p>
                                                    </cds-modal-body>
                                                    <cds-modal-footer>
                                                        ...
                                                        <button type="submit" name="delete_from_account" style="width: 50%; background-color: #FE3B2C; border: 0; color: #ffffff">
                                                            Delete
                                                        </button>
                                                    </cds-modal-footer>
                                                </div>
                                        ...
                                        <button style="
                                                ...
                                                gap: 10px;
                                                margin: 10px 0;
                                            " onclick="onValidateDelete(<?php echo $value['account_id'] ?>, '<?php echo $value['account_name'] ?>')">
    ...
    <form action="account.php" method="post">
        <cds-modal id="modal-add">
            <div style="background-color: #ffffff">
                <cds-modal-header>
                    <p style="font-size: 18px; font-weight: 600; color: #0068a3; margin: 0 30px">ADD ACCOUNT</p>
                </cds-modal-header>
                <cds-modal-body>
                    <div class="cds-ce-demo-devenv--tab-panels" style="margin: 0 30px">
                        <div>
                            ...
                            <div class="input-group mb-3">
                                <input type="text" name="account_name" style="border: 1px solid #0068a3; border-radius: 3px" class="form-control" placeholder="Account Name" />
                            </div>
                        </div>
                        <div>
                            <p style="color: #0068a3; font-size: 16px">Amount</p>
                            <div class="input-group mb-3" style="border: 1px solid #0068a3; border-radius: 3px">
                                <span class="input-group-text">$</span>
                                <input type="number" name="starting_amount" class="form-control" aria-label="Amount (to the nearest dollar)" />
                                <span class="input-group-text">-</span>
                            </div>
                        </div>
                        <div>
                            <p style="color: #0068a3; font-size: 16px">Account Type</p>
                            <select class="form-select" name="account_type" aria-label="Default select example" style="border-radius: 3px; border: 1px solid #0068a3">
                        ...
                        <div>
                            ...
                            <select class="form-select" name="currency_type" aria-label="Default select example" style="border-radius: 3px; border: 1px solid #0068a3">
                ...
                <cds-modal-footer>
                    ...
                    <button type="submit" name="submit_account" style="width: 50%; background-color: #0068a3; border: 0; color: #ffffff">
                        Save
                    </button>
                </cds-modal-footer>
            </div>
                        ...
                        <div style="padding: 10px 20px">
                            ...
                            <div style="display: grid; grid-template-columns: auto auto; gap: 10px; margin-top: 5px">
                                <div>
                                    ...
                                    <div class="input-group mb-3">
                                        <input type="date" name="date" class="form-control" aria-label="Amount (to the nearest dollar)" />
                                    </div>
                                </div>
                                <div>
                                    <p style="color: #0068a3; font-size: 16px">Time</p>
                                    <div class="input-group mb-3">
                                        <input type="time" name="time" class="form-control" aria-label="Amount (to the nearest dollar)" />
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        document.getElementById("modal-add-account").addEventListener("click", () => {
            document.getElementById("modal-add").open = true;
        });
    </script>
</body>
